...
class Field:
    ...
    def rel_db_type(self, engine: str) -> str:
        """Column type for a ForeignKey pointing at this field."""
        return self.sql_type(engine)
...
class BigAutoField(AutoField):
    ...
    def rel_db_type(self, engine: str) -> str:
        return "BIGINT" if engine == "postgresql" else "INTEGER"
...
class AutoUUIDField(UUIDField):
    """
    UUID primary key generated by Pookie (UUIDv7, time-ordered). Takes no arguments.

    Usage:
        id = pookie.AutoUUIDField()
    """

    def __init__(self):
        super().__init__(primary_key=True, editable=False, default=uuid7)

    def sql_definition(self, engine: str) -> str:
        # NOT NULL is explicit: SQLite otherwise accepts NULL in non-INTEGER primary keys
        return f'"{self.get_column_name()}" {self.sql_type(engine)} PRIMARY KEY NOT NULL'
