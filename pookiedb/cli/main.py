...
@cli.command("embed")
@click.option("--model", "-m", "model_name", default=None, help="Only this model (class name).")
@click.option("--reindex", is_flag=True, default=False,
              help="Also rebuild the zvec index (SQLite).")
@click.option("--settings", "-s", default=None, help="Path or dotted module to your settings file.")
def embed(model_name, reindex, settings):
    """Embed rows of searchable models that have no current embedding."""
    _load_settings(settings)
    from pookiedb.search.backfill import searchable_models, embed_missing, reindex as run_reindex

    _banner()
    console.print("[bold cyan]  embed[/bold cyan]\n")
    models = searchable_models()
    if model_name:
        models = [m for m in models if m.__name__ == model_name]
    if not models:
        console.print("[dim]No searchable models with embeddings configured.[/dim]")
        return
    try:
        for model in models:
            count = embed_missing(model)
            console.print(f"  [green]✓[/green] {model.__name__}: {count} row(s) embedded")
            if reindex:
                run_reindex(model)
                console.print(f"  [green]✓[/green] {model.__name__}: zvec index reset")
    except Exception as e:
